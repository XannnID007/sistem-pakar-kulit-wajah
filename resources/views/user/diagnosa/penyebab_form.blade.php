                ...
                @forelse($penyebabs as $index => $penyebab)
                    <div class="question-card">
                        ...
                        <div class="btn-group-choice">
                            {{-- Ubah name menjadi menggunakan ID bukan kode --}}
                            <input type="radio" class="btn-check" name="penyebab[{{ $penyebab->id }}]"
                                id="ya_{{ $penyebab->id }}" value="1.0" required autocomplete="off">
                            <label class="btn btn-outline-primary btn-choice" for="ya_{{ $penyebab->id }}">Ya</label>

                            <input type="radio" class="btn-check" name="penyebab[{{ $penyebab->id }}]"
                                id="ragu_{{ $penyebab->id }}" value="0.5" required autocomplete="off">
                            <label class="btn btn-outline-warning btn-choice"
                                for="ragu_{{ $penyebab->id }}">Ragu-ragu</label>

                            <input type="radio" class="btn-check" name="penyebab[{{ $penyebab->id }}]"
                                id="tidak_{{ $penyebab->id }}" value="0.0" required autocomplete="off">
                            <label class="btn btn-outline-secondary btn-choice"
                                for="tidak_{{ $penyebab->id }}">Tidak</label>
                        </div>
                    </div>
                @empty
